#!/usr/bin/env bash
# process_exports.sh
# Run from the Insta360 folder that contains: Exports/  Ready/  (Raw/ optional)
# Usage: process_exports.sh --dir DIRNAME [other normalize args]
# Moves files to YYYY/DIRNAME-YYYY-MM-DD/ structure
...
command -v normalize_insta360_timestamps.sh >/dev/null || {
  echo "normalize_insta360_timestamps.sh not in PATH" >&2; exit 1; }
...
ORIG_PWD="$PWD"
cd "$EXPORTS"
normalize_insta360_timestamps.sh "${NORMALIZE_ARGS[@]}" || {
