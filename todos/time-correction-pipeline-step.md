...
- `parse_filename_timestamp()` — recognises generic date patterns in filenames (YYYYMMDD_HHMMSS, YYYY-MM-DD, etc.)
...
| `--infer-from-filename` | Filename timestamp (any generic date pattern — YYYYMMDD_HHMMSS, YYYY-MM-DD, etc.) | Camera EXIF is corrupt/wrong, but the filename recorded the correct capture time |
...
| `--companion-extensions .lrv .thm ...` | Also rename companion files with the same stem (from profile) |
...
3. If `--companion-extensions` provided, finds companion files with the same stem and renames them too (same date replacement, different extension)
4. If filename has no parseable date → no-op, emits `@@rename_action=no_date_pattern`
5. Emits `@@renamed_to=<new_filename>` on success, plus `@@companion_renamed=<old>|<new>` for each companion

`media-pipeline.py` reads `@@renamed_to` and updates `active_file` so downstream steps (organize, gyroflow) pick up the new filename. Companion extensions come from the profile's existing `companion_extensions` field.
...
Source files are never modified — only read, then at most moved to an archive folder. When filenames encode timestamps and the time is corrected, the filename date is now wrong.
...
- Companion files (from profile's `companion_extensions`) are renamed alongside the main file — same date replacement, different extension
...
class TimestampReport:
    ...
    # Filename analysis (generic date pattern matching)
    filename_parseable: bool
    filename_date: Optional[datetime]   # parsed from any recognised date pattern
    filename_pattern: Optional[str]     # e.g. "YYYYMMDD_HHMMSS" (the date format, not the prefix)
...
`parse_filename_timestamp()` matches **date patterns**, not prefixes. It looks for date-shaped sequences anywhere in the filename:

| Pattern | Example filename | Extracted |
|---------|-----------------|-----------|
...
| `YYYY-MM-DD_HH-MM-SS` | `Screenshot 2025-05-05 at 13.03.34.png` | `2025-05-05 13:03:34` |
| `YYYYMMDD` (no time) | `DSC_20250505_001.jpg` | `2025-05-05 00:00:00` |

The parser doesn't care about the prefix (`VID_`, `DJI_`, `IMG_`, or anything else). It finds the date portion, records the pattern format used, and preserves everything before/after as prefix/suffix for `build_filename()` to reconstruct.

This means new cameras with novel prefixes (e.g. `INSV_`, `R360_`) work automatically — no code changes needed as long as they use a standard date format somewhere in the filename.

#### Per-profile `filename_timestamp_patterns` (future)

Profiles in `media-profiles.yaml` can optionally specify custom filename date patterns that override generic detection. This is a TODO for when a camera uses a non-standard format that generic detection can't handle. See "What this does NOT cover" section.
...
- `parse_filename_timestamp()` — generic date pattern matching in filenames
...
The lib also needs the reverse operation for renaming. Since the parser records the date format and position (prefix before, suffix after), `build_filename()` replaces just the date portion while preserving everything else:

```python
def build_filename(original_name: str, corrected_date: datetime) -> Optional[str]:
    """Replace the date portion of a filename with the corrected date.

    Detects the same generic date pattern that parse_filename_timestamp() found,
    substitutes the new date in the same format, preserves prefix/suffix/extension.
    e.g. "VID_20250101_120000_00_001.mp4" → "VID_20250505_130334_00_001.mp4"
    """
```

This is prefix-agnostic — works with any filename that has a date, regardless of camera brand or naming convention.
...
7. Tests: filename inference for various date patterns (YYYYMMDD_HHMMSS, YYYYMMDDHHMMSS, YYYYMMDD), interaction with --overwrite-datetimeoriginal, error when no parseable filename
...
19. Tests: reverse-generation for all supported date patterns, no-op when no date in filename, companion file renaming alongside main file
...
- **Per-profile `filename_timestamp_patterns`** — profiles in `media-profiles.yaml` could optionally define custom filename date patterns for cameras that use non-standard formats generic detection can't handle. The generic patterns cover all known cameras today; this is future-proofing. Add to TODO when a real camera needs it.
