...
- (2026-02-27) **Time correction pipeline step** — Extend `fix-media-timestamp.py` to handle camera clock errors (not just timezone). Add `--time-offset`, `--infer-from-filename`, smart warnings. New `rename-file-dates.py` script for updating filename dates. Extract generic transform functions into `lib/transforms.py` for unit-testable building blocks (parse/format EXIF datetimes, parse/build filenames, tz normalisation, offset arithmetic). Surface in macOS app as additional options within Fix Timestamps step (amend time source picker, folder scanning, filename date renaming). Full spec: `todos/time-correction-pipeline-step.md`
